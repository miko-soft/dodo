...
class Auxiliary {
  ...
  _getControllerValue(prop) {
    ...
    try {
      ...
      // Traverse the properties to get the desired value
      let val = this;
      for (const part of propParts) {
        if (val && part in val) {
          val = val[part];
        } else {
          // If any part is undefined, return undefined
          return undefined;
        }
      }
      return val;
 ...
 * For example, the controller's property is this.product.name
 ...
 */
  _setControllerValue(prop, val) {
    try {
      // Split the property string into parts
      const propParts = prop.split('.');
      // Traverse the properties to set the desired value
      let obj = this;
      for (let i = 0; i < propParts.length - 1; i++) {
        const part = propParts[i];
        if (!(part in obj)) {
          // If any part is not found, create an empty object
          obj[part] = {};
        }
        obj = obj[part];
      }
   ...
   *   _solveExpression('(a === b && c === d)', { a: aValue, b: bValue, c: cValue, d: dValue })
   * @param {string} expr - the JS expression
   * @param {object} args - function arguments: {argName: argValue}
   * @returns {boolean|string}
   */
  _solveExpression(expr, args = {}) {
    try {
      // Extract variable names from args object
      const argNames = Object.keys(args);
      // Extract variable values from args object
      const argValues = Object.values(args);

      // Define a safe subset of operators allowed in the expression
      const allowedOperators = ['===', '!==', '&&', '||', '<', '>', '<=', '>=', '+', '-', '*', '/'];

      // Validate the expression to prevent arbitrary code execution
      if (!/^[\w\s()+\-*/%<>=&|!]+$/.test(expr.replace(/[()]/g, ''))) {
        throw new Error(`Invalid characters or syntax in expression: ${expr}`);
      }

      // Replace potentially dangerous characters
      expr = expr.replace(/'/g, '"'); // Replace single quotes with double quotes for safer string handling
      ...
      // Execute function with arguments and get result
      const exprResult = boundFunc(...argValues);

      // Return empty string if result is undefined, null, or NaN
      return exprResult === undefined || exprResult === null || Number.isNaN(exprResult) ? '' : exprResult;
    } catch (err) {
      this._printError(`_solveExpression:: Error evaluating expression "${expr}"`, err);
      console.error(err);
 ...
 */
  _solveTemplateLiteral(text = '', interpolationValues = {}, interpolationMark = '') {
    ...
    if (interpolationMark) {
      const interpolationMarkReg = new RegExp(`\\${interpolationMark}\\{`, 'g');
      text = text.replace(interpolationMarkReg, '${'); // $1{ replaces with ${
    }

    // Regular expression to find all interpolations
    const interpolationReg = /\$\{([^}]+)\}/g;

    // Replace the interpolations with actual values
    text = text.replace(interpolationReg, (match, p1) => {
      try {
        const value = p1.split('.').reduce((acc, part) => acc && acc[part], interpolationValues);
        return value !== undefined && value !== null && value !== NaN ? value : '';
      } catch (err) {
        console.error(`_solveTemplateLiteral:: Error in expression execution "${p1}"`, err);
        return '';
      }
    });
 ...
 * @param {string} str - the string value which should be converted by the pipe: functions
 ...
 */
  _pipeExe(str, pipeOpt) {
    ...
    // Remove 'pipe:' prefix and split the methods
