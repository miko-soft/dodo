...
class Aux {
  ...
  /**
   * Set the $model property's value with <input dd-model="modelName.mprop1.mprop2">.
  ...
  /***** SOLVERS *****/
  /**
   * Replace iteration variable $i with the number. Use only inside dd-for and dd-repeat.
  ...
  _solveInterpolated(txt) {
    ...
    for (const interpolation of interpolations) {
      ...
      // nested interpolation, for example: dd-echo="{{docs.$i.{{fields.$i}}}}"
      if (reg.test(txt)) {
   ...
   */
  _solveMath(txt) {
    const reg = /solveMath\/[^\/]+\//g; // [^\/]+ matches any characte which is not /
    const evs = txt.match(reg); // ['solveMath/0 + 1/', 'solveMath/Math.pow(3,2)/']
    if (!evs) { return txt; }

    for (const ev of evs) {
      const reg2 = /solveMath\/([^\/]+)\//;
      const expression = ev.match(reg2)[1];
      const fja = new Function(`return ${expression};`);
      const result = fja();
  ...
  /***** COMPARISONS *****/
  /**
   * Caclulate comparison with operators ! = < > && ||: dd-show="5 === 3", dd-show="this.age > this.myAge", dd-show="$model.age <= $model.myAge"
   * @param {any} attrVal - dd-show attribute value, for example: 5===3,
   * @returns {boolean}
   */
  ...
  /**
   * Get true/false directly from the controller/model value: dd-show="is_active", dd-show="$model.is_active"
   * Caclulate comparison with $ operators, simillar to mongoDB: dd-show="this.age $eq(18)", dd-show="age $eq(18)", dd-show="age $eq(this.myAge)", dd-show="age $eq($model.myAge)"
   * @param {any} attrVal - dd-show attribute value, for example: is_active, age $gt(this.ctrlProp), age $eq($model.myAge)
   * @returns {boolean}
   */
  ...
  /***** FUNCTIONS *****/
  /**
   * Execute the assignment. For example: $model.age = 3 in dd-click="$model.age = 3" will set model this.$model.age=3
  ...
  * @param {HTMLElement} elem - element where is the dd-... attribute
  ...
  _assignmentExe(assignment, element, event) {
    try {
      ...
      if (/^\$element/.test(val)) { const element_prop = val.split('.')[1] || 'value'; val = element[element_prop]; } // dd-click="$model.x = $element.value"
      else if (/^\$event/.test(val)) { const event_prop = val.split('.')[1] || 'type'; val = event[event_prop]; }  // dd-click="$model.x = $event.type" (rarely used)
      else if (/^\$model/.test(val)) { val = val.replace('$model.', ''); val = this._getModelValue(val); }  // dd-click="$model.x = $model.y"
      else if (/^this\./.test(val)) { val = val.replace('this.', ''); val = this._getControllerValue(val); } // dd-click="$model.x = this.ctrlProp"
      else { val = val; } // dd-click="$model.x = 888"
  ...
  _genElem_define(elem, attrName, attrVal) {
    // hide the original dd-xyz (reference) element
    elem.style.display = 'none';

    let uid = this._uid();

    const ddId = elem.getAttribute(`${attrName}-id`);
    if (!ddId) {
      elem.setAttribute(`${attrName}-id`, uid); // add dd-xyz-id , unique ID (because the page can have multiple elements with [dd-xyz-gen="${attrVal}"] and we need to distinguish them)
    } else {
      uid = ddId; // if the uid is already assigned
    }
  ...
  /**
   * Remove element with the specific dd-xyz-gen and dd-xyz-id attributes.
   * @param {Element} elem - original element
   * @param {string} attrName - attribute name: dd-for, dd-repeat, dd-text
   * @param {string} attrVal - attribute value: 'continent -- append'
   * @returns
  ...
  /**
  * Remove elements which has generated element as parent i.e. if the parent has dd-xyz-gen attribute then delete that parent.
  ...
  * @returns {void}
  */
  _removeParentGenElements(attrName, attrValQuery) {
   ...
   * For example in dd-for="$model.companies.$i0.{{fields.$i1}}" the attrName will be 'dd-for'.
   * As the controller sets this.$model.companies = [...] then attrValQuery will be '$model.companies'.
   * In this case the listed (rendered) elements will be dd-for="$model.companies", dd-for="$model.companies.$i", dd-for="$model.companies.$i0.{{fields.$i1}}"
   * but not dd-for="$model.companies2", dd-for="$model.companies2.$i.name", ...
   * @param {string} attrName - attribute name - 'dd-for'
   ...
   * Here is the issue that this.$model.companies = 'Cloud ltd' will render dd-text="$model.companies" and dd-text="$model.companies2"
   * @param {string} attrName - attribute name - 'dd-for'
   * @param {string|RegExp} attrValQuery - query the attribute value, for example: 'companies' , or /companies\.\$/i
   * @returns {HTMLElement[]}
   */
  ...
  /**
   * Sort elements from higher to lower priority -> 3,2,1,0 . Priority is defined in the attribute value, dd-for="companies --<priority>"
   * @param {HTMLElement[]} elems - array of the elements with specific attribute name
   * @param {string} attrName - attribute name, for example dd-for
   */
  _sortElementsByPriority(elems, attrName) {
    // get priority number from dd-for="companies -- 2"
